...
public final class ToricPosition {
	...
	public double toricDistance(ToricPosition that) {
		return entryVec2s(that).getValue();
	}

	
	// Step 8
	public Vec2d toricVector(ToricPosition that) {
		return entryVec2s(that).getKey().minus(this.toVec2d());
	}

	// Step 8.1
	public Entry<Vec2d, Double> entryVec2s(ToricPosition that) {
		Entry<Vec2d, Double> result = null;
		for (Entry<Vec2d, Double> entry : getDistanceToricPosition(that).entrySet()) {
	...
	// Step  8.1.1
	public Map<Vec2d, Double> getDistanceToricPosition(ToricPosition that) {
		...
		Vec2d trajet1 = that.toVec2d();
		Vec2d trajet2 = initVec2d(that, 0, WORLDHEIGHT, INC);
		Vec2d trajet3 = initVec2d(that, 0, -WORLDHEIGHT,DEC);
		Vec2d trajet4 = initVec2d(that, WORLDWIDTH, 0, INC);
		Vec2d trajet5 = initVec2d(that, -WORLDWIDTH, 0, DEC);
		Vec2d trajet6 = initVec2d(that, WORLDWIDTH, WORLDHEIGHT,INC);
		Vec2d trajet7 = initVec2d(that, -WORLDWIDTH, WORLDHEIGHT,INC);
		Vec2d trajet8 = initVec2d(that, WORLDWIDTH, -WORLDHEIGHT,INC);
		Vec2d trajet9 = initVec2d(that, -WORLDWIDTH, -WORLDHEIGHT,INC);
	...
	private Vec2d initVec2d(ToricPosition position, int width, int height, String operation) {	
		...
		return operation.equals("INC") ? vectorPosition.add(vectorInit) : operation.equals("DEC") ? vectorPosition.add(vectorInit) : this.toVec2d();
	}
